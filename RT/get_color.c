...
static void	update_best_intersection(t_intersection *intersection, \
    ...
    t_intersection *intersection)
{
    ...
    intersection->normal = get_normal_from_map(\
        &rt->scene.objects[obj_id], u, v, intersection->normal);
}
...
static void	accumulate_reflection(t_rt *rt, t_get_color *gc, int nb_rebound, \
    t_pcg32_random *rng, t_vec *pixel)
{
    t_vec   reflection;

    reflection = calculate_recursive_reflection(rt, gc, nb_rebound, rng);
    *pixel = vec_plus(*pixel, reflection);
}
		...
		t_intersection *intersection, int *obj_id)
{
    ...
    while (scene.total_objects)
    {
        ...
        if (obj->is_intersection != NULL && \
            obj->is_intersection(ray, *obj, &local_intersection))
        {
            has_inter = true;
...
t_vec	get_color(t_ray ray, t_rt *rt, int nb_rebound, t_pcg32_random *rng)
{
    ...
    accumulate_reflection(rt, &gc, nb_rebound, rng, &gc.pixel);
	return (gc.pixel);
}
