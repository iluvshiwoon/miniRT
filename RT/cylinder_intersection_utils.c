...
/*   Updated: 2025/07/25 13:17:00 by kgriset          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../miniRT.h"

static void	cyl_prepare_oc_proj(t_cylinder_inter_solve *cy, const t_ray ray, \
...
static void	cyl_prepare_ray_proj(t_cylinder_inter_solve *cy, const t_ray ray, \
    const t_cylinder cylinder)
{
    cy->ray_dir_proj = vec_minus(ray.direction, \
        vec_mult(vec_scal(ray.direction, cylinder.dir), cylinder.dir));
}

static void	cyl_compute_quadratic(t_cylinder_inter_solve *cy, \
    const t_cylinder cylinder)
{
...
static void	cyl_check_first_root(t_cylinder_inter_solve *cy, const t_ray ray, \
    const t_cylinder cylinder)
{
    if (cy->t1 > EPSILON)
    {
